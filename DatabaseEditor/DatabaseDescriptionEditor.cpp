...
void CDatabaseDescriptionEditor::OnOK()
{
	...
	DB_Handle->DatabaseDescription.Name = CW2A(Temp, CP_UTF8);
	m_EditAuthor.GetWindowText(Temp);
	DB_Handle->DatabaseDescription.Author = CW2A(Temp, CP_UTF8);
	m_EditBlock.GetWindowText(Temp);
	DB_Handle->DatabaseDescription.Message = CW2A(Temp, CP_UTF8);
...
BOOL CDatabaseDescriptionEditor::OnInitDialog()
{
	...
	// TODO:  在此添加额外的初始化
	std::wstring w_str;
	StringToWstring(DB_Handle->DatabaseDescription.Name, w_str);
	m_EditName.SetWindowText(w_str.c_str());
	StringToWstring(DB_Handle->DatabaseDescription.Author, w_str);
	m_EditAuthor.SetWindowText(w_str.c_str());
	StringToWstring(DB_Handle->DatabaseDescription.Version, w_str);
	m_EditVersion.SetWindowText(w_str.c_str());
	StringToWstring(DB_Handle->DatabaseDescription.Toolchain, w_str);
	m_EditToolchain.SetWindowText(w_str.c_str());
	StringToWstring(DB_Handle->DatabaseDescription.Message, w_str);
